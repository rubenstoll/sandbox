...
/**
 * Produces {@link ConfigurationValue} annotated fields. It's responsible for
 * supporting conversion between types (mainly from String to any other type
 * required by the user.)
 * 
 * <p>
 * These producers should not be interested where the fields are read from. It's
 * the {@link PropertyResolver} who is responsible for the configuration
 * loading.
 * </p>
 * 
 ...
 * 
 * @author 
 *         http://piotrnowicki.com/2012/06/inject-java-properties-in-java-ee-using
 *         -cdi/
 * 
 */
public class ConfigurationValueProducer {
	 ...
	 * 
	 * <ol>
	 * <li><code>key</code> property of the {@link ConfigurationValue}
	 * annotation (if defined but no key is found - returns null),</li>
	...
	public String getStringConfigValue(InjectionPoint ip) {
		...
		String key = ip.getAnnotated().getAnnotation(ConfigurationValue.class)
				.value();
		boolean isKeyDefined = !key.trim().isEmpty();

		boolean valueRequired = ip.getAnnotated()
		...
		if (value == null && valueRequired) {
			throw new IllegalStateException("No value defined for field: "
					+ fqn + " but field was marked as required.");
		}
